...
const Contact = () => {
  const { data: contactContent } = useSiteContentBySection("contact");

  const getVal = (key: string) => contactContent?.find((c) => c.key === key)?.value;

  const header = (getVal("header") as { title: string; subtitle: string }) || {
    title: "Mari Bersilaturahmi",
    subtitle: "Kami siap membantu menjawab pertanyaan Anda tentang pendaftaran, program pendidikan, dan informasi lainnya.",
  };

  const info = (getVal("info") as Array<{ title: string; content: string }>) || [
    { title: "Alamat", content: "Jl. Raya Demak Kudus Km. 13, Desa Sari, Kec. Gajah, Kab. Demak" },
    { title: "Telepon", content: "+62 812-3456-7890" },
    { title: "Email", content: "info@cahayatasbih.or.id" },
    { title: "Jam Operasional", content: "Senin - Sabtu, 08:00 - 16:00 WIB" },
  ];

  const whatsapp = (getVal("whatsapp") as string) || "+62 812-3456-7890";

  const contactIcons: Record<string, React.ElementType> = {
    Alamat: MapPin, Telepon: Phone, Email: Mail, "Jam Operasional": Clock,
  };
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          <div>
            ...
            <div className="space-y-6 mb-8">
              {info.map((item, index) => {
                const Icon = contactIcons[item.title] || Mail;
                return (
            ...
            <form className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input placeholder="Masukkan nama lengkap" className="h-12" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">No. WhatsApp</label>
                  <Input placeholder="08xxxxxxxxxx" className="h-12" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <Input placeholder="Perihal pesan Anda" className="h-12" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Pesan</label>
                <Textarea placeholder="Tulis pesan Anda di sini..." className="min-h-32 resize-none" />
              </div>
